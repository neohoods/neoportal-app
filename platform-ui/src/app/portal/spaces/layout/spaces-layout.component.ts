import { CommonModule } from '@angular/common';
import { Component, Inject, OnDestroy, OnInit } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { ActivatedRoute, Router, RouterLink, RouterOutlet } from '@angular/router';
import { TranslateModule } from '@ngx-translate/core';
import { tuiAsPortal, TuiPortals } from '@taiga-ui/cdk';
import {
    TuiAppearance,
    TuiAutoColorPipe,
    TuiDataList,
    TuiDropdown,
    TuiDropdownService,
    TuiIcon,
    tuiIconsProvider,
    TuiInitialsPipe,
    TuiRoot
} from '@taiga-ui/core';
import {
    TuiAvatar,
    TuiFade
} from '@taiga-ui/kit';
import { tuiLayoutIconsProvider, TuiNavigation } from '@taiga-ui/layout';
import { FooterComponent } from '../../../components/footer/footer.component';
import { NotificationsPopupComponent } from '../../../components/notifications-popup/notifications-popup.component';
import { ProfileCompletionCheckComponent } from '../../../components/profile-completion-check/profile-completion-check.component';
import { AUTH_SERVICE_TOKEN } from '../../../global.provider';
import { AuthService, UserInfo } from '../../../services/auth.service';
import { ConfigService } from '../../../services/config.service';
import { ApplicationsComponent } from '../../hub/components/applications/applications.component';
import { APPLICATIONS_SERVICE_TOKEN } from '../../hub/hub.provider';
import { MockApplicationsService } from '../../hub/services/mock/applications.service';
import { APIApplicationsService } from '../../hub/services/real-api/applications.service';

interface NavigationItem {
    label: string;
    path: string;
    icon: string;
}

@Component({
    standalone: true,
    selector: 'app-spaces-layout',
    imports: [
        CommonModule,
        FooterComponent,
        RouterOutlet,
        TuiRoot,
        FormsModule,
        RouterLink,
        TuiAppearance,
        TuiAppearance,
        TuiDataList,
        TuiDropdown,
        TuiFade,
        TuiIcon,
        TuiNavigation,
        NotificationsPopupComponent,
        ProfileCompletionCheckComponent,
        TranslateModule,
        TuiAvatar,
        TuiInitialsPipe,
        TuiAutoColorPipe,
        ApplicationsComponent,
    ],
    templateUrl: './spaces-layout.component.html',
    styleUrls: ['./spaces-layout.component.scss'],
    providers: [
        tuiLayoutIconsProvider({ grid: '@tui.align-justify' }),
        tuiIconsProvider({
            tuiIconHome: 'tuiIconHome',
            tuiIconCalendar: 'tuiIconCalendar',
            tuiIconHeart: 'tuiIconHeart',
            tuiIconEye: 'tuiIconEye',
            tuiIconUser: 'tuiIconUser',
            tuiIconSettings: 'tuiIconSettings',
            tuiIconLock: 'tuiIconLock',
            tuiIconLogOut: 'tuiIconLogOut',
            tuiIconCheck: 'tuiIconCheck',
            tuiIconArrowLeft: 'tuiIconArrowLeft',
            tuiIconPlus: 'tuiIconPlus',
            tuiIconToggle: 'tuiIconToggle',
            tuiIconX: 'tuiIconX',
        }),
        TuiDropdownService,
        tuiAsPortal(TuiDropdownService),
        {
            provide: APPLICATIONS_SERVICE_TOKEN,
            useExisting: ConfigService.configuration.useMockApi
                ? MockApplicationsService
                : APIApplicationsService,
        },
    ],
})
export class SpacesLayoutComponent extends TuiPortals implements OnInit, OnDestroy {
    protected expanded = false;
    protected open = false;
    protected switch = false;
    protected readonly routes: any = {};
    totalItemsCurrentlyBorrowed: number = 0;
    totalApprovalRequests: number = 0;
    appConfig = ConfigService.configuration;

    user: UserInfo;

    navigationItems: NavigationItem[] = [
        {
            label: 'Espaces',
            path: '/spaces/list',
            icon: 'tuiIconHome'
        },
        {
            label: 'Mes réservations',
            path: '/spaces/reservations',
            icon: 'tuiIconCalendar'
        },
        {
            label: 'Favoris',
            path: '/spaces/favorites',
            icon: 'tuiIconHeart'
        }
    ];

    constructor(
        @Inject(AUTH_SERVICE_TOKEN) private authService: AuthService,
        private route: ActivatedRoute,
        private router: Router,
    ) {
        super();
        this.user = this.authService.getCurrentUserInfo();
    }

    ngOnInit() {
        // Force layout recalculation when switching from admin to hub
        setTimeout(() => {
            window.dispatchEvent(new Event('resize'));
        }, 0);
    }

    ngOnDestroy() {
    }

    navigateTo(path: string) {
        this.router.navigate([path]);
    }
}