...
public class MatrixAssistantMCPServer {
        ...
        /**
         * Lists MCP tools available for a specific user (filtered by roles)
         * Admin tools are only included if the user has admin role
         */
        public List<MatrixMCPModels.MCPTool> listToolsForUser(MatrixAssistantAuthContext authContext) {
                List<MatrixMCPModels.MCPTool> allTools = listTools();
                if (allTools.isEmpty()) {
                        return allTools;
                }

                // Check if user is admin
                boolean isAdmin = isAdminUser(authContext);

                // Filter tools: exclude admin tools if user is not admin
                return allTools.stream()
                                .filter(tool -> {
                                        // Include all non-admin tools
                                        if (!tool.getName().startsWith("admin_")) {
                                                return true;
                                        }
                                        // Include admin tools only if user is admin
                                        return isAdmin;
                                })
        ...
        public MatrixMCPModels.MCPToolResult callTool(String toolName, Map<String, Object> arguments,
                ...
                // Get trace ID, span ID, conversation trace ID, and room ID from MDC for logs
                String traceId = MDC.get("traceId");
                String spanId = MDC.get("spanId");
                String conversationTraceId = MDC.get("conversationTraceId");
                String roomId = MDC.get("roomId");
                log.info("Calling MCP tool: {} with arguments: {} for user: {} [traceId={}, spanId={}, conversationTraceId={}, roomId={}]",
                                toolName, arguments, authContext.getMatrixUserId(),
                                traceId != null ? traceId : "N/A", 
                                spanId != null ? spanId : "N/A",
                                conversationTraceId != null ? conversationTraceId : "N/A",
                                roomId != null ? roomId : "N/A");
                ...
                try {
                        return transactionTemplate.execute(status -> {
                                try {
                                        ...
                                        // NOTE: For reservation tools, the router already ensures we're in a DM
                                        // So we skip the DM check for reservation tools to avoid false positives
                                        if (requiresDM(toolName) && !isReservationTool(toolName)) {
                                                // Check if conversation is public (DM required) - check FIRST before
                                                // trying to get user
                                                if (authContext.isConversationPublic()) {
                                                        log.warn("Tool {} requires DM but conversation is public (roomId={}, isDirectMessage={})",
                                                                        toolName, authContext.getRoomId(),
                                        ...
                                        } else if (isReservationTool(toolName)) {
                                                // Reservation tools: router already verified DM, but log for
                                                // debugging
                                                log.debug("Reservation tool {} called - router already verified DM (isDirectMessage={}, roomId={})",
                                                                toolName, authContext.isDirectMessage(),
                                                                authContext.getRoomId());

                                                // Get user entity - required for DM tools
                                                try {
                                                        userEntity = authContext.hasUser()
                                                                        ? authContext.getAuthenticatedUser()
                                                                        : getUser(authContext.getMatrixUserId());
                        ...
                        log.error("Transaction rollback error calling MCP tool {}: {} [traceId={}, spanId={}]",
                                        toolName,
                                        e.getMessage(),
        ...
        /**
         * Checks if a tool is a reservation tool
         * Reservation tools are already checked by the router (which ensures DM)
         * So we skip the DM check in MCP server for these tools
         */
        private boolean isReservationTool(String toolName) {
                return "create_reservation".equals(toolName) ||
                                "list_my_reservations".equals(toolName) ||
                                "get_reservation_access_code".equals(toolName) ||
                                "generate_payment_link".equals(toolName) ||
                                "check_space_availability".equals(toolName) ||
                                "list_spaces".equals(toolName);
        }
         ...
         */
        public boolean isAdminUser(MatrixAssistantAuthContext authContext) {
                if (authContext.hasUser()) {
                        return needAdminRole(authContext.getAuthenticatedUser());
                }
