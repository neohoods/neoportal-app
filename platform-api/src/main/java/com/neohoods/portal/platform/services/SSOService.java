...
public class SSOService {
    ...
    public URI generateSSOLoginUrl() {
        ...
        try {
            ...
            return new URI(uriString + separator + "ui_locales=fr");
    ...
    public Mono<Boolean> tokenExchange(ServerWebExchange exchange, String state, String authorizationCode) {
        return Mono.fromCallable(() -> {
            ...
            if (!tokenResponse.indicatesSuccess()) {
                log.error("Token response failure: "
                        + tokenResponse.toErrorResponse().toJSONObject().toJSONString());
                return false;
            }
            ...
            try {
                String idTokenJson = objectMapper
                        .writeValueAsString(idToken.getJWTClaimsSet().toJSONObject());
                log.info("idToken: {}", idTokenJson);
            } catch (JsonProcessingException e) {
                log.warn("Failed to serialize idToken to JSON: {}", e.getMessage());
                log.info("idToken: {}", idToken.getJWTClaimsSet().toJSONObject());
            }
            ...
            try {
                ...
                if (user == null) {
                    ...
                    // Generate a random password for SSO users (they won't use it, but DB requires it)
                    ...
                    // Create new user entity (without notificationSettings - will be created after save)
