...
public class MatrixBotService {
    ...
    private String localBotUserId;

    @Value("${neohoods.portal.matrix.local-bot.permanent-token:}")
    private String localBotPermanentToken;

    @Value("${neohoods.portal.matrix.local-bot.avatar-url:}")
    private String botAvatarUrl;

    @Value("${neohoods.portal.matrix.local-bot.display-name:Bot Neohoods}")
     ...
     * 2. localBotPermanentToken (from MATRIX_LOCAL_BOT_PERMANENT_TOKEN)
    ...
    private Optional<ApiClient> getMatrixAccessToken() {
        ...
        // configured
        else if (localBotEnabled && localBotPermanentToken != null && !localBotPermanentToken.isEmpty()) {
            log.info("✅ Using permanent token for local bot user: {} (token prefix: {})", localBotUserId,
                    localBotPermanentToken.substring(0, Math.min(10, localBotPermanentToken.length())));
            accessTokenOpt = Optional.of(localBotPermanentToken);
        } else if (localBotEnabled) {
            ...
            accessTokenOpt = createPermanentTokenForUser(localBotUserId);
        ...
        if (accessTokenOpt.isEmpty()) {
            ...
            if (accessTokenOpt.isPresent()) {
                ...
                log.error("   2. POST https://matrix.neohoods.com/_synapse/admin/v1/users/{}/login",
                        localBotUserId != null ? localBotUserId : "@bot:chat.neohoods.com");
                log.error(
    ...
    public Optional<String> getBotUserId() {
        ...
        if (localBotEnabled && localBotUserId != null && !localBotUserId.isEmpty()) {
            log.debug("Using configured local bot user ID: {}", localBotUserId);
            return Optional.of(localBotUserId);
        }
    ...
    public boolean sendMessage(String roomId, String message) {
        try {
            ...
            Optional<String> botUserIdOpt = getBotUserId();
            ...
            Optional<String> membership = getUserRoomMembership(botUserId, decodedRoomId);
            if (!membership.isPresent() || !"join".equals(membership.get())) {
                log.warn("Bot {} is not a member of room {} (membership: {}). Attempting to join...", botUserId,
                        decodedRoomId, membership.orElse("none"));
                // Try to join the room
                boolean joined = joinRoomAsBot(decodedRoomId);
                if (!joined) {
                    log.error("Cannot send message: bot {} failed to join room {}", botUserId, decodedRoomId);
                    return false;
                }
                // Re-check membership after joining
                membership = getUserRoomMembership(botUserId, decodedRoomId);
                if (!membership.isPresent() || !"join".equals(membership.get())) {
                    log.error("Bot {} still not a member of room {} after join attempt (membership: {})", botUserId,
                            decodedRoomId, membership.orElse("none"));
                    return false;
                }
                log.info("Bot {} successfully joined room {} before sending message", botUserId, decodedRoomId);
            }
            ...
            try {
                ...
                if (e.getCode() == 500 && e.getMessage() != null && e.getMessage().contains("Internal server error")) {
                    log.error(
                            ...
                            localBotEnabled && localBotPermanentToken != null && !localBotPermanentToken.isEmpty()
                                    ? "permanent token"
                                    : "OAuth2 token",
                            localBotEnabled,
                            localBotPermanentToken != null && !localBotPermanentToken.isEmpty());
                    log.error(
                            "💡 SOLUTION: Create a permanent token via Synapse Admin API and configure it in MATRIX_LOCAL_BOT_PERMANENT_TOKEN");
                    log.error(
                            "   Example: Use setup-bot-permanent-token.sh or create token via: POST /_synapse/admin/v1/users/@bot:chat.neohoods.com/login");
                }
                throw e;
            }
            ...
            if (e.getCode() == 500) {
                Optional<String> botUserIdOpt = getBotUserId();
                if (botUserIdOpt.isPresent()) {
                    Optional<String> membership = getUserRoomMembership(botUserIdOpt.get(), roomId);
                    log.error("Bot {} membership status in room {}: {}", botUserIdOpt.get(), roomId,
    ...
    public void handleNewUser(UserEntity user) {
        ...
        try {
            ...
            // Invite to space
            // Note: Room invitations are handled by MatrixBotInitializationService
     ...
     * For example: @bot:chat.neohoods.com -> bot
     * 
     * @param matrixUserId The Matrix user ID (e.g., @user:server.com)
     ...
     * For example: @bot:chat.neohoods.com -> %40bot%3Achat.neohoods.com
     * 
    ...
    public boolean updateMatrixUserProfile(String matrixUserId, UserEntity user) {
        try {
            ...
            ApiClient apiClient = apiClientOpt.get();
            // Note: Profile updates in Matrix are typically done via the Profile API or
            // state events
            // For now, we'll skip profile updates as the AccountManagementApi doesn't have
            // setAccountData
            // The displayname is already set during user creation via createMatrixUser
            // Email and phone updates would require using the Profile API or admin API
            log.info("Matrix user profile update skipped for {} (displayname set during creation)", matrixUserId);
            return true;
        } catch (Exception e) {
    ...
    public boolean updateBotAvatar() {
        ...
        String botUserId = botUserIdOpt.get();
        log.info("Updating avatar for bot user: {} from URL: {}", botUserId, botAvatarUrl);
        ...
        try {
            ...
            String normalizedUrl = normalizeHomeserverUrl(homeserverUrl);
            String currentAvatarUrl = getCurrentBotAvatar(botUserId, normalizedUrl);
            ...
            String profileUrl = UriComponentsBuilder.fromHttpUrl(normalizedUrl)
                    .pathSegment("_matrix", "client", "v3", "profile", botUserId, "avatar_url")
                    .build()
                    .toUriString();
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                log.info("Successfully updated bot avatar for {} to {}", botUserId, mxcUrl);
                return true;
            } else {
            ...
            log.error("Error updating bot avatar for {}: {}", botUserId, e.getMessage(), e);
            return false;
        }
    }
    ...
    public boolean updateBotDisplayName() {
        ...
        String botUserId = botUserIdOpt.get();
        log.info("Updating display name for bot user: {} to: {}", botUserId, botDisplayName);
        ...
        try {
            ...
            String normalizedUrl = normalizeHomeserverUrl(homeserverUrl);
            String profileUrl = UriComponentsBuilder.fromHttpUrl(normalizedUrl)
                    .pathSegment("_matrix", "client", "v3", "profile", botUserId, "displayname")
                    .build()
                    .toUriString();
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                log.info("Successfully updated bot display name for {} to {}", botUserId, botDisplayName);
                return true;
            } else {
            ...
            log.error("Error updating bot display name for {}: {}", botUserId, e.getMessage(), e);
            return false;
        }
    }
     ...
     * The bot invites itself and then accepts the invitation
     */
    public boolean joinRoomAsBot(String roomId) {
        try {
            ...
            Optional<String> membership = getUserRoomMembership(botUserId, roomId);
            if (membership.isPresent() && "join".equals(membership.get())) {
                log.debug("Bot {} is already a member of room {}", botUserId, roomId);
                return true;
            }
            ...
            // Try to invite the bot to the room
            try {
                InviteUserRequest inviteRequest = new InviteUserRequest();
                inviteRequest.setUserId(botUserId);
                membershipApi.inviteUser(decodedRoomId, inviteRequest);
                log.debug("Bot {} invited to room {}", botUserId, decodedRoomId);
            } catch (ApiException e) {
                if (e.getCode() == 403) {
                    // Bot might already be invited or have permission to join directly
                    log.debug("Bot {} already invited or has permission to join room {}", botUserId, decodedRoomId);
                } else {
                    log.warn("Failed to invite bot {} to room {}: HTTP {}", botUserId, decodedRoomId, e.getCode());
                }
            }

            // After inviting, check if bot is now a member
            // The invitation should automatically make the bot a member if it has
            // permission
            // Wait a bit and check membership status
            try {
                Thread.sleep(500); // Small delay for invitation to process
                Optional<String> botMembership = getUserRoomMembership(botUserId, decodedRoomId);
                if (botMembership.isPresent() && "join".equals(botMembership.get())) {
                    log.info("Bot {} successfully joined room {}", botUserId, decodedRoomId);
                    return true;
                } else if (botMembership.isPresent() && "invite".equals(botMembership.get())) {
                    log.info("Bot {} is invited to room {} (may need manual approval)", botUserId, decodedRoomId);
                    return true; // Invited, will be able to join
                } else {
                    log.warn("Bot {} is not a member of room {} after invitation", botUserId, decodedRoomId);
                    return false;
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                log.warn("Interrupted while checking bot membership");
                return false;
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (currentRoomAvatar != null && !currentRoomAvatar.isEmpty()) {
                if (imageUrl.startsWith("http://") || imageUrl.startsWith("https://")) {
                    ...
                    // We can't compare them, so assume it's already correct to avoid re-uploading
                    log.debug(
                            "Room avatar is already set (current: {}), skipping update to avoid re-uploading same image",
                            currentRoomAvatar);
                    return true; // Already set, skip update
     ...
     */
    @SuppressWarnings("unchecked")
    private String getCurrentBotAvatar(String botUserId, String normalizedUrl) {
        try {
            String profileUrl = UriComponentsBuilder.fromHttpUrl(normalizedUrl)
                    .pathSegment("_matrix", "client", "v3", "profile", botUserId, "avatar_url")
                    .build()
