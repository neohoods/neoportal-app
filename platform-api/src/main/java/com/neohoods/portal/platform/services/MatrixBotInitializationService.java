...
public class MatrixBotInitializationService {
    ...
    private void doInitializeBot() {
        ...
        try {
            ...
            matrixBotService.updateBotAvatar();
            matrixBotService.updateBotDisplayName();
    ...
    private boolean checkSpace() {
        try {
            boolean exists = matrixBotService.checkSpaceExists(spaceId);
            if (!exists) {
    ...
    private void createDefaultRooms(List<RoomConfig> rooms, InitializationStats stats) {
        ...
        Map<String, String> existingRooms = matrixBotService.getExistingRoomsInSpace(spaceId);
            ...
            try {
                ...
                if (existingRoomId != null) {
                    ...
                    if (room.getImage() != null && !room.getImage().isEmpty()) {
                        ...
                        boolean avatarUpdated = matrixBotService.updateRoomAvatar(existingRoomId, room.getImage());
                        if (avatarUpdated) {
                ...
                Optional<String> roomId = matrixBotService.createRoomInSpace(
                        room.getName(),
                        room.getDescription(),
                        room.getImage(),
                        spaceId,
    ...
    private void synchronizeUsers(List<RoomConfig> rooms, InitializationStats stats) {
        ...
        for (int i = 0; i < allUsers.size(); i++) {
            ...
            try {
                ...
                Optional<String> createdUserId = matrixBotService.createMatrixUser(user);
                ...
                if (createdUserId.isPresent()) {
                    ...
                    // (the actual creation logic is in MatrixBotService)
                    created++;
                ...
                // Update user profile
                boolean profileUpdated = matrixBotService.updateMatrixUserProfile(matrixUserId, user);
                if (profileUpdated) {
                    updated++;
                }
                ...
                boolean invitedToSpace = matrixBotService.inviteUserToSpace(matrixUserId);
                if (invitedToSpace) {
    ...
    private void countPendingInvitations(List<RoomConfig> rooms, InitializationStats stats) {
        try {
            ...
            Map<String, String> allRoomsMap = matrixBotService.getExistingRoomsInSpace(spaceId);
            ...
            for (UserEntity user : allUsers) {
                ...
                for (Map.Entry<String, String> roomEntry : allRoomsMap.entrySet()) {
                    ...
                    Optional<String> membership = matrixBotService.getUserRoomMembership(matrixUserId, roomId);
    ...
    private RoomInvitationResult inviteUserToRooms(UserEntity user, String matrixUserId, List<RoomConfig> rooms) {
        ...
        if (user.getType() == UserType.PROPERTY_MANAGEMENT) {
            ...
            if (syndicRoomIdStr == null) {
                Optional<String> syndicRoomIdOpt = matrixBotService.getRoomIdByName(syndicRoomName, spaceId);
                syndicRoomIdStr = syndicRoomIdOpt.orElse(null);
            }
            if (syndicRoomIdStr != null) {
                // Check if user is already a member or invited before inviting
                Optional<String> membership = matrixBotService.getUserRoomMembership(matrixUserId, syndicRoomIdStr);
                if (membership.isPresent()) {
                    ...
                    log.info("Inviting syndic user {} to {} room ({})", matrixUserId, syndicRoomName,
                            syndicRoomIdStr);
                    boolean invited = matrixBotService.inviteUserToRoomWithNotifications(
                            matrixUserId,
                            syndicRoomIdStr,
        ...
        for (RoomConfig room : rooms) {
            if (Boolean.TRUE.equals(room.getAutoJoin())) {
                ...
                if (roomIdStr == null) {
                    ...
                    Optional<String> roomIdOpt = matrixBotService.getRoomIdByName(room.getName(), spaceId);
                    roomIdStr = roomIdOpt.orElse(null);
                }
                if (roomIdStr != null) {
                    // Check if user is already a member or invited before inviting
                    Optional<String> membership = matrixBotService.getUserRoomMembership(matrixUserId, roomIdStr);
                    if (membership.isPresent()) {
                        if ("join".equals(membership.get())) {
        ...
        if (unitName != null) {
            Optional<String> building = matrixBotService.parseBuildingFromUnitName(unitName);
            if (building.isPresent()) {
                ...
                if (buildingRoomIdStr == null) {
                    Optional<String> buildingRoomIdOpt = matrixBotService.getRoomIdByName(buildingRoomName,
                            spaceId);
                    buildingRoomIdStr = buildingRoomIdOpt.orElse(null);
                }
                if (buildingRoomIdStr != null) {
                    // Check if user is already a member or invited before inviting
                    Optional<String> membership = matrixBotService.getUserRoomMembership(matrixUserId,
                            buildingRoomIdStr);
                    if (membership.isPresent()) {
                        if ("join".equals(membership.get())) {
                            log.info(
        ...
        if (user.getType() == UserType.OWNER) {
            ...
            if (proprioRoomIdStr == null) {
                Optional<String> proprioRoomIdOpt = matrixBotService.getRoomIdByName("Proprio", spaceId);
    ...
    private void sendInitializationSummary(List<RoomConfig> rooms, InitializationStats stats) {
        try {
            ...
            Optional<String> itRoomId = matrixBotService.getRoomIdByName(IT_ROOM_NAME, spaceId);
            if (itRoomId.isEmpty()) {
                // Create IT room
                // IT room is a restricted room (allowGuests=false)
                itRoomId = matrixBotService.createRoomInSpace(
                        IT_ROOM_NAME,
                        "Room pour les notifications techniques du bot Matrix",
                        null,
                        spaceId,
            ...
            boolean botJoined = matrixBotService.joinRoomAsBot(itRoomId.get());
            if (botJoined) {
            ...
            if (!adminUsersList.isEmpty()) {
                ...
                for (UserEntity adminUser : adminUsersList) {
                    try {
                        ...
                        Optional<String> matrixUserIdOpt = matrixBotService.findUserInMatrix(adminUser);
            ...
            summary.append("🎉 **Initialisation du bot Matrix terminée avec succès !**\n\n");
            ...
            // Send message
            matrixBotService.sendMessage(itRoomId.get(), summary.toString());
